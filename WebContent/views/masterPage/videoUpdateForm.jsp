<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.ArrayList, master.Class.model.vo.*, lecture.model.vo.Lecture, master.Video.model.vo.Video"%>

<%
	Video v = (Video) request.getAttribute("video");
	ArrayList<Classs> list = (ArrayList<Classs>) request.getAttribute("list");
	ArrayList<Lecture> list1 = (ArrayList<Lecture>) request.getAttribute("list1");
%>
...
<body>
	...
	<form action="<%=contextPath%>/update.vi" method="post">
			...
			<div class="row">
				...
				<div id=line>
					...
					<table class="question">
						<input type="hidden" value="<%=v.getV_code()%>" name="v_code">
					...
					<script>
						$(document).on(
							"change",".l_title", // l_title 값이 바뀔때 마다 실행된다.
							function() {
														...
														success : function(list) {
															$.each(
																	...
																	function(index,value) {
																		if (index.includes("Classs")) {
																				var $select = $(".c_title");
																					$select.html(""); // 선택될 때마다 값 비우기
																					for (var i = 0; i < value.length; i++) {
																						var $c = $("<option>").text(value[i].c_title);
																						$select.append($c);
																						/* console.log(value[i].c_title); */
																					}
																				}
																			});
																	}
