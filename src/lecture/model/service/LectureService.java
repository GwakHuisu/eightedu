...
public class LectureService {
	...
	public int buyLecture(int l_code, int loginUserCode) {
		...
		int price = new LectureDao().getPrice(con, l_code);
		
		int t_code = new LectureDao().getTcode(con, l_code);
		
		int result2 = new LectureDao().buyLecture(con, price, loginUserCode);
		if(price > 0 && result2 >0) {

			int updateOrderList = new LectureDao().updateOrder(con, l_code, t_code,loginUserCode);
